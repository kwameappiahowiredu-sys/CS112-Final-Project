...
def calculate_distance_km(start_latitude, start_longitude, end_latitude, end_longitude):
    ...
    end_latitude_rad = np.radians(end_latitude)
    latitude_change_rad = np.radians(end_latitude - start_latitude)
    longitude_change_rad = np.radians(end_longitude - start_longitude)

    haversine_value = (
        np.sin(latitude_change_rad / 2) ** 2
        + np.cos(start_latitude_rad)
        * np.cos(end_latitude_rad)
        * np.sin(longitude_change_rad / 2) ** 2
    )
...
def clean_lines(lines, utilities, substations):
    ...
    geodesic = calculate_distance_km(src["Latitude"].values, src["Longitude"].values,
