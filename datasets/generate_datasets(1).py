...
def calculate_distance_km(start_latitude, start_longitude, end_latitude, end_longitude):
    ...
    latitude_change_rad = math.radians(end_latitude - start_latitude)
    longitude_change_rad = math.radians(end_longitude - start_longitude)

    haversine_value = (
        math.sin(latitude_change_rad / 2) ** 2
        + math.cos(start_latitude_rad)
        * math.cos(end_latitude_rad)
        * math.sin(longitude_change_rad / 2) ** 2
    )
    ...
    for i, a in enumerate(ids_in_region):
        for b in ids_in_region[i + 1:]:
            if random.random() < 0.55:
                ...
                dist = round(calculate_distance_km(sub_a[5], sub_a[6], sub_b[5], sub_b[6])
...
for i in range(len(region_names) - 1):
    ...
    sub_a, sub_b = sub_by_id[a], sub_by_id[b]
    dist = round(calculate_distance_km(sub_a[5], sub_a[6], sub_b[5], sub_b[6]) * 1.15, 1)
    lines.append([
...
for src_name, dst_name, utility_id in border_links:
    ...
    sub_a, sub_b = sub_by_id[a], sub_by_id[b]
    dist = round(calculate_distance_km(sub_a[5], sub_a[6], sub_b[5], sub_b[6]) * 1.1, 1)
    lines.append([
...
with open(RAW_DIR / "utilities.csv", "w", newline="") as f:
    w = csv.writer(f)
    w.writerow(["Utility ID", "Name", "Alias", "Code", "Type", "Country", "Active"])
    w.writerows(utilities)

with open(RAW_DIR / "substations.csv", "w", newline="") as f:
    w = csv.writer(f)
...
with open(RAW_DIR / "lines.csv", "w", newline="") as f:
    w = csv.writer(f)
